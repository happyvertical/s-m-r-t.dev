<script lang="ts">
	...
	const sampleArticle = {
		...
		excerpt:
			'Learn how to build scalable applications with the SMRT framework. This guide covers installation, configuration, and best practices.',
...
<ModuleTabs
	...
	badges={['v0.29.34', 'Governance', 'Contributions', 'Prompt Registry']}
>
	{#snippet docs()}
		<section id="overview">
			<h2>Overview</h2>
			<p>
				...
				<strong>reviews and corrections</strong>,
				<strong>versioning with frozen transparency</strong>, a dedicated
				<code>content_assets</code> noun join, and prompt-registered thumbnail strategies.
			</p>

			<h3>Key Features</h3>
			<ul>
				<li>
					STI hierarchy: <code>Article</code>, <code>ContentDocument</code>, <code>Mirror</code>
					share <code>contents</code>
				</li>
				<li>
					Persisted governance: <code>ContentGovernancePolicy</code>,
					<code>ContentGovernanceProfile</code>, <code>ContentGovernanceAssignment</code>
				</li>
				<li>AI reviews via <code>runReviewAction()</code> with fingerprint staleness detection</li>
				<li>
					Post-publication corrections: <code>ContentCorrection</code> (correction / retraction / update
					/ clarification)
				</li>
				<li>
					Version snapshots: <code>ContentVersion</code> with frozen transparency on publication
				</li>
				<li>
					Contributions: <code>ContentContribution</code> + Contributor / Type / Revision / Attachment
				</li>
				<li>
  ...
  smrtContentThumbnailAIGeneratePrompt,
		...
		<section id="sti-models">
			...
			<p>
				Content uses single-table inheritance keyed by the qualified discriminator — e.g. <code
					>@happyvertical/smrt-content:Article</code
				>
				— and tenant scoping is optional (<code>tenantId=null</code> means global content).
			</p>
			...
			<table>
				...
				<tbody>
					...
					<tr
						><td><code>Article</code></td><td><code>@happyvertical/smrt-content:Article</code></td
						></tr
					>
					<tr
						><td><code>ContentDocument</code></td><td
							><code>@happyvertical/smrt-content:ContentDocument</code></td
						></tr
					>
					<tr
						><td><code>Mirror</code></td><td><code>@happyvertical/smrt-content:Mirror</code></td
						></tr
					>
				</tbody>
			</table>
...
await content.getReferences();`}
		...
		<section id="governance">
			...
			<p>
				...
				<code>ContentGovernanceAssignment</code> (type/variant → profile, feature flags). DB rows merge
				over static config.
			</p>
...
await article.save();  // throws ValidationError if required reviews are missing/stale`}
		...
		<section id="contributions">
			...
			<table>
				...
				<tbody>
					<tr
						><td><code>ContentContribution</code></td><td
							>Held submission with lifecycle: submitted → approved / rejected / withdrawn →
							promoted</td
						></tr
					>
					<tr
						><td><code>ContentContributor</code></td><td
							>Profile resolved by email; trust level (standard / trusted / blocked)</td
						></tr
					>
					<tr
						><td><code>ContentContributionType</code></td><td
							>Channel configuration: rules and promotion mapping</td
						></tr
					>
					<tr
						><td><code>ContentContributionRevision</code></td><td
							>Revision history on a single submission</td
						></tr
					>
					<tr
						><td><code>ContentContributionAttachment</code></td><td
							>Held file; becomes an Asset on promotion</td
						></tr
					>
				</tbody>
			</table>
...
const livePreview       = await article.previewTransparencyAction();`}
  ...
  subtitle: 'Civic News',
			...
			<h3>static-map</h3>
			<p>
				Requires <code>metadata.latitude</code> + <code>metadata.longitude</code>; uses unary
				<code>+</code> for strict parsing.
			</p>
			<CodeBlock
  ...
  limit: 100,
		...
		<section id="prompt-registry">
			...
			<p>
				...
				<a href="/modules/smrt-prompts"><code>@happyvertical/smrt-prompts</code></a> so tenants can override
			...
			<table>
				...
				<tbody>
					...
					<tr
						><td><code>smrtContent.applyCorrection</code></td><td>Correction synthesis flows</td
						></tr
					>
					<tr
						><td><code>smrtContent.thumbnail.aiGenerate</code></td><td
							><code>ThumbnailGenerator</code> strategy <code>'ai-generate'</code></td
						></tr
					>
				</tbody>
			</table>
			...
			<p>
				...
				free-form <code>metadata</code> blob (may carry tenant-private configuration or coordinates unrelated
  ...
  doc.updateMetadata({ revision: (meta.revision ?? 0) + 1 });
		...
		<section id="chat-integration">
			...
			<p>
				...
				endpoint gracefully returns <code>session: null</code> with a notice when chat tables are
				not yet provisioned. Editor components support
...
content.isInCategory('technology'); // true`}
		...
		<section id="gotchas">
			...
			<ul>
				<li>
					<strong>Publish readiness enforcement</strong>: <code>save()</code> throws
					<code>ValidationError</code>
					if blocking requirements aren't met when setting status to <code>'published'</code>.
				</li>
				<li>
					<strong>Stale reviews</strong>: detected by fingerprint mismatch on the content's
					reviewable state.
				</li>
				<li>
					<strong>Transparency snapshots</strong>: published transparency is frozen in
					<code>ContentVersion.metadata.transparency</code> — use published for public display, preview
					for editors.
				</li>
				<li>
					<strong>Metadata is primary extension pattern</strong>: prefer the JSON
					<code>metadata</code> field over additional class fields.
				</li>
				<li>
					<strong>Static map coordinates</strong>: uses unary <code>+</code> for strict parsing
					(rejects <code>"45invalid"</code>).
				</li>
				<li>
					<strong>content_assets, not AssetAssociation</strong>: use the dedicated noun join for
					content-owned assets; <code>AssetAssociation</code> is for generic / provenance only.
				</li>
			</ul>
		</section>
		...
		<section id="related">
			...
			<ul>
				...
				<li>
					<a href="/modules/smrt-assets">smrt-assets</a> — Asset model + AssetRuntime;
					<code>content_assets</code> references Asset
				</li>
				<li>
					<a href="/modules/smrt-images">smrt-images</a> — Image extends Asset; sets thumbnails on content
				</li>
				<li>
					<a href="/modules/smrt-prompts">smrt-prompts</a> — Tenant-overridable AI prompt registry
				</li>
	...
	{#snippet components()}
		<section>
			...
			<p>
				The <code>@happyvertical/smrt-content</code> package includes 30+ Svelte 5 components covering
				content lists, editing, governance, transparency reporting, and contribution management. The examples
				below render a few of the most common surfaces.
			</p>
...
<ArticleCard {article} showTags showDate showAuthor />`}
			...
			<ul>
				<li>
					<strong>Content management</strong>: <code>ContentList</code>, <code>ContentEditor</code>,
					<code>GovernedContentEditor</code>, <code>ContentAgentChat</code>,
					<code>ContentBodyEditor</code>, <code>ContentBodyRenderer</code>,
					<code>ArticleCard</code>, <code>ArticleList</code>, <code>Markdown</code>
				</li>
				<li>
					<strong>Governance</strong>: <code>ContentGovernanceManager</code>,
					<code>ContentGovernancePanel</code>, <code>ContentGovernancePolicyEditor</code>,
					<code>ContentGovernanceProfileEditor</code>,
					<code>ContentGovernanceAssignmentEditor</code>, <code>ContentTransparencyReport</code>
				</li>
				<li>
					<strong>Contributions</strong>: <code>ContentContributionForm</code>,
					<code>ContentContributionInbox</code>, <code>ContentContributionPortal</code>,
					<code>ContentContributionTypeManager</code>, <code>ContentContributorManager</code>
				</li>
			</ul>
