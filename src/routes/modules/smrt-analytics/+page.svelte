...
<ModulePage
	...
	badges={['v0.24.12', 'GA4', 'Plausible', 'Matomo', 'Prompt Registry']}
>
	<section>
		<h2>Overview</h2>
		<p>
		...
		<aside>
			...
			<ul>
				...
				<li>STI everywhere (<code>tableStrategy: 'sti'</code>): <code>AnalyticsProperty</code>, <code>AnalyticsDataStream</code>, <code>AnalyticsEvent</code>, <code>AnalyticsReport</code></li>
				<li>Server-side event tracking with retry queue (<code>pending</code>/<code>sent</code>/<code>failed</code>)</li>
				<li>Scheduled reports (<code>once</code>/<code>daily</code>/<code>weekly</code>/<code>monthly</code>) with cached result data</li>
				<li><strong>Three registered prompts</strong> via smrt-prompts — tenant-overridable templates, models, and params</li>
				<li><strong>UI slot registry</strong>: <code>analytics-summary</code>, <code>events-table</code>, <code>property-info</code>, <code>property-status-badge</code>, <code>stat-card</code>, <code>trend-badge</code></li>
				<li>Tenancy-fallback: omits <code>tenantId</code> so <code>withTenant()</code> context applies automatically</li>
			</ul>
		</aside>
	</section>
...
await event.markSent();`}
  ...
  // smrt-prompts: smrtAnalytics.report.analyzeResults (now a single AI call;
  // returned response shape includes "insights" mirroring "analysis").
  async analyzeResults(): Promise<{ analysis: string; insights: string }>
...
if (report.isDue()) {
  ...
  console.log(insights);
	...
	<section>
		...
		<p>Three of the four AI methods on this package route through the smrt-prompts registry; <code>isPerformingWell()</code> still uses the inline <code>is()</code> shortcut.</p>
		<CodeBlock
...
// { analysis, insights }  // insights mirrors analysis in the response shape
...
// true if key metrics improving`}
	...
	<section>
		...
		<p>Three prompts are registered at module-load time via <code>definePrompt()</code> from <a href="/modules/smrt-prompts">@happyvertical/smrt-prompts</a> and re-exported from the package root for tenant override targeting.</p>
		<table>
			<thead>
				<tr>
					<th>Key</th>
			...
			<tbody>
				<tr>
					...
					<td><code>period</code>, <code>propertyDisplayName</code>, <code>propertyProvider</code></td>
				</tr>
				<tr>
					<td><code>smrtAnalytics.report.analyzeResults</code></td>
					<td><code>AnalyticsReport.analyzeResults()</code></td>
					<td><code>reportName</code>, <code>reportDimensions</code>, <code>reportMetrics</code>, <code>dateRangeStart</code>, <code>dateRangeEnd</code>, <code>rowCount</code>, <code>reportData</code></td>
				</tr>
				<tr>
		...
		<ul>
			<li><code>id</code>, <code>propertyId</code>, <code>tenantId</code>, <code>externalId</code> — internal FKs / UUIDs that link to identifying records</li>
			<li><code>apiSecret</code>, <code>measurementId</code>, <code>siteDomain</code> — provider credentials and platform-specific IDs (GA4 API secrets, Matomo <code>idSite</code>, custom <code>G-XXXX</code> measurement IDs)</li>
			<li><code>providerMetadata</code> — extensible JSON that may contain credentials or account IDs</li>
			<li><code>lastError</code>, raw <code>dimensionFilter</code> / <code>metricFilter</code> JSON — error strings (may contain auth tokens) and filter expressions that may reference cookie IDs, user-pseudo-IDs, or IP-derived geos</li>
		</ul>

		<h3 id="resultdata-contract"><code>resultData</code> is forwarded verbatim — callers must de-PII</h3>
		<aside>
			<p>
			...
			<ul>
				<li>Exclude PII-bearing dimensions before persisting <code>resultData</code> (e.g. don't request <code>userPseudoId</code> as a GA4 dimension).</li>
				<li>Apply a column allowlist at the call site.</li>
				<li>Override the prompt template via <code>PromptOverride</code> to redact rows server-side.</li>
			</ul>
		</aside>
	</section>
	...
	<section>
		...
		<p>Svelte components live in <code>src/svelte/</code> and auto-register with <code>ModuleUIRegistry</code> from <code>@happyvertical/smrt-svelte/registry</code> when <code>@happyvertical/smrt-analytics/svelte</code> is imported. Slot declarations are exported via the <code>./ui</code> subpath.</p>
		<p>Slots: <code>analytics-summary</code>, <code>events-table</code>, <code>property-info</code>, <code>property-status-badge</code>, <code>stat-card</code>, <code>trend-badge</code> (see <code>ANALYTICS_UI_SLOTS</code> for icons / categories / display order).</p>
		<CodeBlock
		...
		<div>
			<aside>
				...
				<ul>
					...
					<li>De-PII <code>resultData</code> at the call site before persisting — <a href="#resultdata-contract">forward-verbatim contract</a></li>
					<li>Override prompt templates via <code>PromptOverride</code> for tenant-specific tone/redaction</li>
			...
			<aside>
				<h3>DON'Ts</h3>
				<ul>
					<li>Don't store API secrets unencrypted long-term (GA4 secrets currently stored plaintext — TODO)</li>
					<li>Don't retry failed events indefinitely (use a max-retries cutoff)</li>
					<li>Don't send PII in event parameters</li>
					<li>Don't request <code>userPseudoId</code>, <code>clientId</code>, or IP-derived geos in report dimensions if <code>analyzeResults()</code> will run on the data</li>
