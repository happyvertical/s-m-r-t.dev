...
<ModulePage
	...
	badges={['v0.24.12', 'Catalog', 'Triple-Consumption Template']}
>
	<section>
		<h2>Overview</h2>
		<p>
			<strong>smrt-products</strong> is the SMRT <strong>reference template</strong> for the
			triple-consumption pattern. The same codebase is consumed three ways without forking:
		</p>
		<ol>
			<li><strong>NPM library</strong> — import models, collections, and helpers directly. The default <code>npm run build</code> emits the published library surface to <code>dist/lib</code>; run <code>npm run build:all</code> only when you also need standalone or federation bundles.</li>
			<li><strong>Module federation</strong> — runtime component sharing across micro-frontends (still marked <strong>experimental</strong> — APIs may change).</li>
			<li><strong>Standalone API</strong> — <code>startRestServer([Product, Category])</code> boots an Express server with auto-generated REST routes and OpenAPI.</li>
		</ol>
		<p>
			Virtual modules generated by the SMRT Vite plugin power the integration:
			<code>@happyvertical/smrt-client</code>, <code>@happyvertical/smrt-types</code>,
			<code>@happyvertical/smrt-routes</code>, <code>@happyvertical/smrt-mcp</code>, and
			<code>@happyvertical/smrt-manifest</code>.
		</p>
		<aside>
			<p><strong>Key Features:</strong></p>
			<ul>
				<li>Product (STI) and Category (STI, hierarchical with <code>parentId</code>, <code>level</code>, <code>productCount</code>) models</li>
				<li><strong>ProductAsset</strong> — dedicated owned-asset join in the <code>product_assets</code> table with <code>relationship</code> and <code>sortOrder</code>. Intentionally not tenant-scoped because <code>Product</code> isn't tenant-scoped in this package</li>
				...
				<li>Svelte 5 stores using runes (<code>$state</code>, <code>$derived</code>, <code>$effect</code>) — separate server/client store files for SSR safety</li>
				<li>Auto-generated REST APIs, MCP tools, and CLI commands via <code>@smrt()</code></li>
				<li>ProductCollection helpers: <code>findByManufacturer()</code>, <code>findInStock()</code>, <code>getAssets()</code> / <code>addAsset()</code> / <code>removeAsset()</code></li>
			</ul>
		</aside>
...
console.log('Laptops with 32GB+ RAM:', highRamLaptops.length);`}
	...
	<section>
		...
		<article>
			...
			<ul>
				...
				<li>Don't rely on <code>Object.assign</code> in constructors — assign each property explicitly so decorators see them</li>
			</ul>
		</article>
	</section>
	...
	<section>
		...
		<p>This package is the canonical reference for SMRT's <strong>triple-consumption</strong> architecture — the same source consumed in three deployment modes:</p>
		<CodeBlock
...
import { ProductCard } from '@happyvertical/smrt-products/svelte';

// 3. Standalone REST server — boot an Express app with auto-generated routes
import { startRestServer } from '@happyvertical/smrt-core';
import { Product, Category } from '@happyvertical/smrt-products';

await startRestServer([Product, Category], { port: 3000 });`}
			language="typescript"
		/>
		<p><strong>Build commands:</strong></p>
		<ul>
			<li><code>npm run build</code> — emits the published library surface to <code>dist/lib</code>. This is what npm consumers read.</li>
			<li><code>npm run build:all</code> — additionally produces standalone and federation bundles.</li>
		</ul>
	</section>
	...
	<section>
		...
		<p>
			...
			<code>asset_associations</code>) because it is a noun-owned asset relationship — see
			the asset-ownership convention in <code>docs/content/standards.md §7</code>.
		</p>
	</section>
