...
<ModulePage
	...
	badges={['v0.24.12', 'Properties', 'Zones']}
>
	<section>
		<h2>Overview</h2>
		<p>
			<strong>smrt-properties</strong> manages digital properties (websites, apps, publications)
			and their hierarchical zones for content and ad placement. <code>Property</code> is
			STI-enabled with domain, URL, and optional repository/owner links. <code>Zone</code> forms an
			arbitrarily nested tree within each property, with in-memory depth caching to prevent N+1
			queries during tree operations.
		</p>
		<aside>
			<p><strong>Key Features:</strong></p>
			<ul>
				<li><code>Property</code> (STI) with <code>domain</code>, <code>url</code>, <code>status</code> (active/inactive/pending)</li>
				<li>Zone hierarchy via <code>parentId</code> with tree operations and cycle detection</li>
				<li>Dimension tracking (width/height, nullable independently) and <code>allowedFormats</code> per zone</li>
				<li>Empty <code>allowedFormats</code> = all formats allowed (no restrictions)</li>
				<li><code>deleteZone(cascade=false)</code> orphans children to parent, doesn't delete them</li>
		...
		<p>
			...
			<code>ZoneCollection.findWithGlobals(tenantId)</code> to include global zones with tenant
			ones — common for shared zone templates.
		</p>
	</section>
...
await zones.findWithGlobals(tenantId);      // tenant + global zones`}
	...
	<section>
		<h2>Gotchas</h2>
		<ul>
			<li><strong>Empty <code>allowedFormats</code> = all formats</strong>: an empty array means no restrictions, NOT "no formats"</li>
			<li><strong>Zone dimensions nullable independently</strong>: check <code>hasDimensions()</code> before using <code>width</code>/<code>height</code></li>
			<li><strong><code>deleteZone(cascade=false)</code></strong>: doesn't delete children — moves them to the parent (orphan pattern)</li>
		...
		<div>
			<div>
				...
				<ul>
					...
					<li>Use <code>getTree()</code> over repeated <code>getChildren()</code> calls — it's depth-cached</li>
					<li>Use <code>moveZone()</code> for reparenting — it validates against descendant cycles</li>
					<li>Validate formats with <code>isFormatAllowed()</code> before assignment</li>
					<li>Override <code>summarize()</code> at the tenant level via smrt-prompts instead of subclassing</li>
				</ul>
			</div>
			<div>
				<h3>DON'Ts</h3>
				<ul>
					...
					<li>Don't query zones repeatedly in loops — use <code>getTree()</code> or batch operations</li>
					<li>Don't put PII or sensitive analytics IDs in <code>metadata</code> if you plan to surface <code>summarize()</code> publicly</li>
				</ul>
			</div>
		</div>
	</section>
