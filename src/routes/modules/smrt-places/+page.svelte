...
<ModulePage
	...
	badges={['v0.24.12', 'Geography', 'Hierarchy']}
>
	<section>
		<h2>Overview</h2>
		<p>
		...
		<aside>
			...
			<ul>
				...
				<li>Automatic geocoding integration (OpenStreetMap + Google Maps via <code>@happyvertical/geo</code>)</li>
				...
				<li>Type system via <code>PlaceType</code> (country, city, building, zone, room, region, ...)</li>
				<li><strong>PlaceAsset</strong> (new): dedicated owned-asset join in <code>place_assets</code> with <code>relationship</code> and <code>sortOrder</code></li>
				<li>JSON metadata storage for custom attributes</li>
				<li>Multi-tenant via <code>findWithGlobals(tenantId)</code> — returns tenant + global (<code>tenantId=null</code>) places</li>
			</ul>
		</aside>
		...
		<p>
			...
			<code>tenantId = null</code> is a global place visible to every tenant — useful for shared
			geo reference data (countries, well-known landmarks). Tenant-specific places live alongside
			and are retrieved together via <code>findWithGlobals(tenantId)</code>.
		</p>
	</section>
...
pnpm add @happyvertical/smrt-places`}
		...
		<p>
			...
			geocoding providers (OpenStreetMap, Google Maps), <code>@happyvertical/sql</code> for
			database operations, and optionally <code>@happyvertical/smrt-tenancy</code> for multi-tenant
			scoping.
		</p>
	</section>
...
const placeFromCoords = await places.lookupOrCreate(
  ...
  { geoProvider: 'openstreetmap', typeSlug: 'address', createIfNotFound: true }
);`}
			language="typescript"
		/>
		...
		<CodeBlock
			code={`// Haversine distance, sorted nearest-first
const nearby = await places.searchByProximity(37.7749, -122.4194, 5 /* km */);
nearby.forEach(p => console.log(p.name, p.distance, 'km'));

// Coordinate lookup (default 0.0001 degrees ~= 11m tolerance at the equator)
const exact = await places.findByCoordinates(37.7749, -122.4194);`}
			language="typescript"
		/>
  ...
  getGeoData(): GeoData
	...
	<section>
		...
		<p>
			...
			reserve generic <code>AssetAssociation</code> for cross-cutting / provenance links that don't
			belong to a single owner.
		</p>
		<CodeBlock
			code={`// Owner-side helpers
await place.addAsset(photoAssetId, { relationship: 'gallery', sortOrder: 0 });
const gallery = await place.getAssets({ relationship: 'gallery' });
await place.removeAsset(photoAssetId);

// Collection-side bulk reads
const byPlace = await places.getAssets([sfId, nycId]); // Map<placeId, Asset[]>`}
			language="typescript"
		/>
	</section>
	...
	<section>
		...
		<table>
			...
			<tbody>
				<tr><td><code>lookupOrCreate(query, opts)</code></td><td>DB first → geocode if not found → create. Organic growth pattern.</td></tr>
				<tr><td><code>findByCoordinates(lat, lng, threshold?)</code></td><td>Default 0.0001° ≈ 11m tolerance at equator (less at poles)</td></tr>
				<tr><td><code>searchByProximity(lat, lng, radiusKm)</code></td><td>Haversine distance, sorted by proximity</td></tr>
				<tr><td><code>findWithGlobals(tenantId)</code></td><td>Tenant + global (<code>tenantId=null</code>) places</td></tr>
				<tr><td><code>getRootPlaces()</code>, <code>getByType(slug)</code></td><td>Hierarchy + type queries</td></tr>
				<tr><td><code>getAssets([ids])</code> / per-instance <code>getAssets()</code> / <code>addAsset()</code> / <code>removeAsset()</code></td><td>Owned-asset helpers backed by <code>place_assets</code></td></tr>
			</tbody>
		</table>
	</section>
...
const zone = await places.create({
  ...
  metadata: {
	...
	<section>
		<h2>Gotchas</h2>
		<ul>
			<li><strong>Geo providers</strong>: Google Maps requires <code>GOOGLE_MAPS_API_KEY</code>; OpenStreetMap is default and free (rate-limited)</li>
			<li><strong>Abstract places allowed</strong>: lat/lng are nullable — check <code>hasCoordinates()</code> before using them</li>
			<li><strong>Coordinate tolerance varies by latitude</strong>: 0.0001° ≈ 11m at the equator, less at the poles</li>
			<li><strong>Optional tenancy</strong> with nullable <code>tenantId</code> — use <code>findWithGlobals(tenantId)</code> for tenant + global places</li>
			<li>Use <code>PlaceAsset</code> (<code>place_assets</code>) for owned assets; reserve <code>AssetAssociation</code> for generic/provenance links</li>
		</ul>
	</section>
