...
<ModulePage
	name="smrt-dev-mcp"
	description="Development MCP server for SMRT framework integration."
	badges={['v0.19.0', 'MCP', 'Developer Tools']}
>
	<section>
		<h2>Overview</h2>
		<p>
			<strong>smrt-dev-mcp</strong> is an MCP server that exposes SMRT objects to Claude Code, enabling
			natural language interactions with your database objects directly from Claude chat.
		</p>
	</section>

	<section>
		<h2>Installation</h2>
		<CodeBlock code={`npm install @happyvertical/smrt-dev-mcp`} language="bash" />
	</section>
...
npx smrt-dev-mcp start`}
	...
	<section>
		...
		<p>Once running, Claude Code can interact with your objects:</p>
		<CodeBlock
			code={`User: List all open issues
