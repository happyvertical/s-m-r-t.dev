...
<article class="prose">
		...
		<h2>Agent Class</h2>
		<p>Agents are designed for long-running processes, scheduled tasks, and autonomous operations requiring state management.</p>
		<pre><code>{`import { Agent } from '@happyvertical/smrt-agents';
...
└──────────────────────────────────────────────────────────┘`}</code></pre>
		...
		<table>
			...
			<tbody>
				...
				<tr><td><code>shutdown()</code></td><td>Cleanup resources, handle graceful termination</td></tr>
...
await agent.execute();`}</code></pre>

		<h2>Agent Status</h2>
		<pre><code>{`type AgentStatusType = 'idle' | 'initializing' | 'running' | 'error' | 'shutdown';`}</code></pre>
...
class Crawler extends Agent {
  ...
  async run(): Promise<void> {
    ...
    await this.save(); // Persist changes
		...
		<pre><code>{`// Save config for a UI slot
...
const exported = await agent.exportConfig({ includeSecrets: false });`}</code></pre>

		<h2>Agent Communication</h2>
		<p>Agents communicate via the <a href="/docs/agents/dispatch">DispatchBus</a>. Built-in methods:</p>
		<pre><code>{`class Fiscus extends Agent {
  protected config = {};
      ...
      await this.recordRevenue(payload);
		...
		<table>
			...
			<tbody>
				...
				<tr><td><code>handleDispatch(payload, metadata)</code></td><td>Override to process incoming dispatches</td></tr>
				<tr><td><code>processDispatches()</code></td><td>Process all pending dispatches for this agent</td></tr>
			</tbody>
		</table>

		<h2>Agent Interests</h2>
		<p>Agents can declaratively query objects they're interested in:</p>
		<pre><code>{`const agent = new MyAgent({
  name: 'my-agent',
  interests: {
...
for (const { type, data, name, handled } of items) {
  console.log(\`\${type} from "\${name}": action=\${handled?.action}\`);
		...
		<table>
			...
			<tbody>
				<tr><td><code>filter</code></td><td>Record&lt;string, any&gt;</td><td>SQL filter with operators</td></tr>
				<tr><td><code>query</code></td><td>(tableName) =&gt; [sql, params]</td><td>Custom SQL for complex patterns</td></tr>
				...
				<tr><td><code>qualify</code></td><td>(items) =&gt; Promise&lt;items&gt;</td><td>Post-filter async processing</td></tr>
				<tr><td><code>handler</code></td><td>(item, agent) =&gt; any</td><td>Action for each matched item</td></tr>
			</tbody>
		</table>
...
AgentUIRegistry.register('Praeco', 'sources', SourcesPanel);`}</code></pre>
		...
		<pre><code>{`async initialize(): Promise<this> {
  ...
  return this;
		...
		<pre><code>{`async run(): Promise<void> {
