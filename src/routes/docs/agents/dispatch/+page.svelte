...
const bus = await createDispatchBus({ db: existingDb });`}</code
		></pre>
...
// List subscriptions
const subs = await bus.listSubscriptions('Fiscus');`}</code
		></pre>
...
// Unregister
bus.off('campaign.completed', handler);`}</code
		></pre>
	...
	<table>
		...
		<tbody>
			<tr
				><td><code>campaign.*</code></td><td>campaign.started, campaign.completed</td><td
					>campaign.summer.started</td
				></tr
			>
			<tr
				><td><code>agent.*.completed</code></td><td
					>agent.suasor.completed, agent.fiscus.completed</td
				><td>agent.suasor.started</td></tr
			>
			<tr><td><code>*</code></td><td>All events</td><td>-</td></tr>
			<tr
				><td><code>*.*.completed</code></td><td>agent.suasor.completed, task.backup.completed</td
				><td>agent.completed</td></tr
			>
		</tbody>
	</table>
	<p>Rules:</p>
...
console.log(\`Processed \${count} dispatches\`);`}</code
		></pre>
...
// Then process again
await bus.process('Fiscus', handler);`}</code
		></pre>
	<p>
		Retry resets <code>status</code> from <code>failed</code> to <code>pending</code>. The
		<code>attempts</code> counter is preserved.
	</p>
...
console.log(\`Deleted \${result.failedDeleted} failed\`);`}</code
		></pre>
...
// Get by ID
const dispatch = await bus.get('dispatch-uuid');`}</code
		></pre>
  ...
  logger.info('Processing order', {
	...
	<table>
		...
		<tbody>
			...
			<tr
				><td><code>metadata</code></td><td>Record&lt;string, unknown&gt;</td><td
					>Custom trace data</td
				></tr
			>
		</tbody>
	</table>
	...
	<table>
		...
		<tbody>
			...
			<tr
				><td><code>status</code></td><td>TEXT</td><td>pending, processing, completed, failed</td
				></tr
			>
	...
	<table>
		...
		<tbody>
			<tr><td><code>id</code></td><td>TEXT</td><td>Unique identifier</td></tr>
			<tr
				><td><code>signal_type</code></td><td>TEXT</td><td>Pattern to match (supports wildcards)</td
				></tr
			>
...
// Bad - unknown origin
await bus.emit('event', payload);`}</code
		></pre>
