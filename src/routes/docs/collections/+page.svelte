...
<article class="prose">
		...
		<p class="lead">
			SmrtCollection provides a standardized interface for managing sets of persistent SmrtObject instances.
			It handles database setup, schema generation, and provides a fluent API for querying objects.
		</p>

		<h2>Creating Collections</h2>

		<h3>Static Factory (Required)</h3>
		<p>Collections <strong>must</strong> be created using the static <code>create()</code> factory method.</p>
		<pre><code>{`import { SmrtCollection, SmrtObject, smrt } from '@happyvertical/smrt-core';
...
const collection = new ProductCollection(options); // Error`}</code></pre>

		<h2>CRUD Operations</h2>

		<h3>Create</h3>
		<p>Creates a new object instance, generates an ID, and saves to database.</p>
		<pre><code>{`const product = await products.create({
  name: 'Widget',
  price: 29.99
});
// product.id is now a UUID
// product is saved to database`}</code></pre>

		<p>For STI (Single Table Inheritance) classes, specify <code>_meta_type</code>:</p>
		<pre><code>{`const meeting = await events.create({
  _meta_type: 'Meeting',
  title: 'Team Standup',
  roomNumber: '101'
});`}</code></pre>
...
// By filter
const product = await products.get({ name: 'Widget' });`}</code></pre>

		<h3>Update</h3>
		<p>Objects are updated via the <code>save()</code> method after modification:</p>
		<pre><code>{`const product = await products.get('uuid-123');
product.price = 39.99;
await product.save();`}</code></pre>

		<h3>Delete</h3>
		<pre><code>{`const product = await products.get('uuid-123');
await product.delete();`}</code></pre>

		<h3>getOrUpsert</h3>
		<p>Get existing object or create if not found:</p>
		<pre><code>{`const product = await products.getOrUpsert(
  ...
  { name: 'Widget Pro', price: 49.99 }  // defaults for new object
		...
		<pre><code>{`const results = await products.list({
  where: { status: 'active' },
  ...
  offset: 0
		...
		<table>
			...
			<tbody>
				<tr><td><code>=</code> (default)</td><td><code>{`{ status: 'active' }`}</code></td><td>status = 'active'</td></tr>
				<tr><td><code>&gt;</code></td><td><code>{`{ 'price >': 100 }`}</code></td><td>price &gt; 100</td></tr>
				<tr><td><code>&lt;</code></td><td><code>{`{ 'price <': 50 }`}</code></td><td>price &lt; 50</td></tr>
				<tr><td><code>&gt;=</code></td><td><code>{`{ 'stock >=': 10 }`}</code></td><td>stock &gt;= 10</td></tr>
				<tr><td><code>&lt;=</code></td><td><code>{`{ 'rating <=': 4.5 }`}</code></td><td>rating &lt;= 4.5</td></tr>
				<tr><td><code>!=</code></td><td><code>{`{ 'deleted_at !=': null }`}</code></td><td>deleted_at != NULL</td></tr>
				<tr><td><code>in</code></td><td><code>{`{ 'category in': ['A', 'B'] }`}</code></td><td>category IN ('A', 'B')</td></tr>
				<tr><td><code>like</code></td><td><code>{`{ 'name like': '%widget%' }`}</code></td><td>name LIKE '%widget%'</td></tr>
			</tbody>
		</table>
...
// Multiple fields
await products.list({ orderBy: ['category ASC', 'price DESC'] });`}</code></pre>

		<h3>Pagination</h3>
		<pre><code>{`await products.list({
  limit: 20,
  offset: 40  // Skip first 40 records
});`}</code></pre>

		<h3>count()</h3>
		<pre><code>{`const total = await products.count({});
const active = await products.count({ where: { status: 'active' } });`}</code></pre>

		<h2>Batch Operations</h2>

		<h3>listByIds()</h3>
		<p>Fetch multiple objects by ID in a single query:</p>
		<pre><code>{`// Single query, not N queries
const profiles = await profileCollection.listByIds([
  'id-1', 'id-2', 'id-3', 'id-4', 'id-5'
		...
		<pre><code>{`// N+1 problem: 1 + 100 queries
...
for (const order of orders) {
  ...
  const product = order.getRelated('productId');   // Already loaded
		...
		<pre><code>{`// Get all customer IDs from orders
...
// Build lookup map
const customerMap = new Map(customers.map(c => [c.id, c]));`}</code></pre>

		<h2>Raw SQL Queries</h2>
		<p>For complex queries that the standard API cannot express:</p>
		<pre><code>{`// NOT EXISTS pattern
...
\`, ['Electronics', 100]);`}</code></pre>
		...
		<pre><code>{`const collection = await ProductCollection.create({
  db: { type: 'sqlite', url: 'products.db' }
});
...
const collection = await ProductCollection.create({
  ...
  // Commits on success, rolls back on error
		...
		<pre><code>{`const tx = await db.beginTransaction();
try {
  await tx.insert('orders', { id: 'ord-1', total: 100 });
  await tx.insert('order_items', { orderId: 'ord-1', product: 'Widget' });
  await tx.commit();
  ...
  throw error;
		...
		<pre><code>{`// Remember a parsing strategy
...
// Wrong
const collection = new MyCollection(options);`}</code></pre>

		<h3>2. Define _itemClass</h3>
		<pre><code>{`class MyCollection extends SmrtCollection<MyObject> {
  static readonly _itemClass = MyObject; // Required
}`}</code></pre>

		<h3>3. Use listByIds for Batch Lookups</h3>
		<pre><code>{`// Good - single query
...
for (const order of orders) {
  await order.loadRelated('customerId');
		...
		<pre><code>{`// Parent creates database
const parent = await ParentCollection.create({ db: 'app.db' });

// Child shares connection
const child = await ChildCollection.create({ db: parent.options.db });`}</code></pre>
	</article>
